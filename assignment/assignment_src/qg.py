...
class QG_MODEL:
    ...
    def two_layer(self,g,zeta,out):
        ...
        self.vmax = -1
        for ilev in [0,1]:
            # need to imlement helmholtz solver
            # read 
            # psi = helmholtz(g,zeta[ilev,:,:],psi[:,:],c_wrk, lambda)

            if ilev == 0:    # barotropic mode tendency equation 
                out=0.
            elif ilev == 1:  # baroclinic mode tendency equation 
                out=0. 
    ...
    def exponential(self,g,x,dummy=None):
        # Dummy argument used for compatibility with rossby and two_layer functions:
        #     (in this case, we do not need a work space) 
        return(x) 
    ...
    def initialize(self):
        ...
        if md == 'rossby' or md =='two_layer':
            ...
            if md == 'two_layer': 
                ...
                self.lambda_par=RTYPE(self.nl_parameter(md,'lambda_par',0))
        elif md == 'exponential':
            self.end_time=RTYPE(self.nl_parameter(md,'End',10)) 
            self.lx=1.
            self.ly=1. 
